...
**サーバーもデータベースも使いません。** ブラウザがZotero APIを直接読み、集計も描画もその場で行います。あなたの業績データが第三者のサーバーを経由することはありません。
...
の4形式に対応。Wordにそのまま貼れる
プレーンテキストで出力します。

和文論文では著者名を日本語で、
欧文スタイルでは `Y. Arakawa` のように
イニシャル表記へ自動変換します。
...
<p align="center">
  <img src="docs/images/04-search.png" alt="意味検索の結果" width="100%">
  <em>「高齢者の見守りセンサ」の検索結果。類似度つきで上位30件を表示</em>
</p>
...
<p align="center">
  ...
  <em>年をクリックすると区分ごとに整理された一覧が開く。右上でスタイルを選んで書き出せる</em>
</p>
...
**初回の意味検索には時間がかかります。** 約120MBのモデルを読み込み、全論文をベクトル化します（1,000件で1〜2分）。2回目以降はブラウザに保存されるため数秒です。
...
- **ソース別の取得方法と実測カバレッジ**
  ...
  | ソース | 実測件数 | APIキー |
  |---|---|---|
  | researchmap（5カテゴリ） | **925** | 不要 |
  | OpenAlex | 407 | 不要 |
  | DBLP | 269 | 不要 |
  | ORCID | 182 | 不要 |
  | Google Scholar | — | **APIなし** |

  日本の研究者は **researchmap だけで9割**揃います。逆に国際的な索引だけでは全体の約40%しか埋まりません（国内会議・研究会が載っていないため）。
...
このツールは、実際に1,050件の業績をZoteroへ集約した経験から作られています。
手引きの数値はすべて実測値です。
